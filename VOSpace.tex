...
\previousversion[http://www.ivoa.net/documents/VOSpace/20180502/index.html]{PR-VOSpace-2.1-20180502}
...
\subsubsection{Immutable nodes}
\label{subsubsec:immutable nodes}

A VOSpace service MAY support immutable nodes. Content of immutable nodes can't be changed and they are identified by adding the standard property ivo://ivoa.net/vospace/core\#immutable set to true.

In particular:

\begin{itemize}
\item moveNode and deleteNode operations are not allowed
\item for data nodes, transfers that write data into the node (pushToVoSpace or pullToVoSpace operations) are not allowed
\item if the node is a container node, child nodes cannot be added or removed; children of an immutable node inherit this property
\item for link nodes, it is not possible to change the target element
\item properties that are not readOnly can still be edited
\end{itemize}

A PermissionDenied fault is thrown if forbidden operations are attempted.

Typical use cases for this are locking directories for DOI purposes or storing data for long term preservation.
...
\begin{itemize}
    \item ivo://ivoa.net/vospace/core\#title MAY be used as the property URI denoting a name given to the resource
    \item ivo://ivoa.net/vospace/core\#creator MAY be used as the property URI denoting an entity primarily responsible for making the resource
    \item ivo://ivoa.net/vospace/core\#subject MAY be used as the property URI denoting the topic of the resource
    \item ivo://ivoa.net/vospace/core\#description MAY be used as the property URI denoting an account of the resource
    \item ivo://ivoa.net/vospace/core\#publisher MAY be used as the property URI denoting an entity responsible for making the resource available
    \item ivo://ivoa.net/vospace/core\#contributor MAY be used as the property URI denoting an entity responsible for making contributions to this resource
    \item ivo://ivoa.net/vospace/core\#date MAY be used as the property URI denoting a point or period of time associated with an event in the lifecycle of the resource
    \item ivo://ivoa.net/vospace/core\#type MAY be used as the property URI denoting the nature or genre of the resource
    \item ivo://ivoa.net/vospace/core\#format MAY be used as the property URI denoting the file format, physical medium, or dimensions of the resource
    \item ivo://ivoa.net/vospace/core\#identifier MAY be used as the property URI denoting an unambiguous reference to the resource within a given context
    \item ivo://ivoa.net/vospace/core\#source MAY be used as the property URI denoting a related resource from which the described resource is derived
    \item ivo://ivoa.net/vospace/core\#language MAY be used as the property URI denoting a language of the resource
    \item ivo://ivoa.net/vospace/core\#relation MAY be used as the property URI denoting a related resource
    \item ivo://ivoa.net/vospace/core\#coverage MAY be used as the property URI denoting the spatial or temporal topic of the resource, the spatial applicability of the resource, or the jurisdiction under which the resource is relevant
    \item ivo://ivoa.net/vospace/core\#rights MAY be used as the property URI denoting information about rights held in and over the resource
    \item ivo://ivoa.net/vospace/core\#availableSpace MAY be used as the property URI denoting the amount of space available within a container
    \item ivo://ivoa.net/vospace/core\#groupread MAY be used as the property URI denoting the list of groups which can only read this resource (read-only)
    \item ivo://ivoa.net/vospace/core\#groupwrite MAY be used as the property URI denoting the list of groups which can read and write to this resource (read-write)
    \item ivo://ivoa.net/vospace/core\#publicread MAY be used as the property URI denoting whether this resource is world readable (anon-read-only)
    \item ivo://ivoa.net/vospace/core\#quota MAY be used as the property URI denoting the value of a system quota on the resource
    \item ivo://ivoa.net/vospace/core\#length MAY be used as the property URI denoting the length or size of a resource
    \item ivo://ivoa.net/vospace/core\#mtime MAY be used as the property URI denoting the data modification time
    \item ivo://ivoa.net/vospace/core\#ctime MAY be used as the property URI denoting status change (aka metadata modification) time
    \item ivo://ivoa.net/vospace/core\#btime MAY be used as the property URI denoting initial creation time
\end{itemize}
...
23 & Standard properties are represented by the specified URIs & ~\ref{subsubsec:standard properties} \\ \hline
23 & Data imported with the default View is treated as a binary BLOB & ~\ref{subsubsec:default views} \\ \hline
24 & Data exported with the default export View is returned in the most appropriate format & ~\ref{subsubsec:default views} \\ \hline
25 & An archive format View on a ContainerNode provides access to the archive contents as children nodes of the container & ~\ref{subsubsec:container views}* \\ \hline
26 & An archive format View specified in a data export operation on a ContainerNode will package the contents of the container and all its child nodes & ~\ref{subsubsec:container views}* \\ \hline
\end{tabular}

\hskip-2.0cm\begin{tabular}{ | p {1cm} | p {11cm} | p {4cm} | }
\hline
\textbf{Item} & \textbf{Description} & \textbf{Occurs in section(s)} \\ \hline
27 & A Protocol has elements: uri and and endpoint & ~\ref{subsec:protocols} \\ \hline
28 & Standard Protocols are represented by the specified URIs & ~\ref{subsubsec:standard protocols} \\ \hline
29 & A Transfer UWS Job representation has elements: target, direction, view, protocol and keepBytes & ~\ref{subsec:transfers} \\ \hline
30 & A Transfer results representation has elements: target, direction, view, and protocol & ~\ref{subsec:transfers} \\ \hline
31 & A server responds with a fault if it is unable to handle any of the requested protocols in a data transfer & ~\ref{subsubsec:service-initiated transfers}, ~\ref{subsubsec:client-initiated transfers} \\ \hline
32 & A server uses each requested protocol only once in a data transfer & ~\ref{subsubsec:service-initiated transfers}, ~\ref{subsubsec:client-initiated transfers} \\ \hline
33 & A data transfer is complete once a specified protocol is successful & ~\ref{subsubsec:service-initiated transfers}, ~\ref{subsubsec:client-initiated transfers} \\ \hline
34 & A data transfer has failed if none of the specified protocols has been successful & ~\ref{subsubsec:service-initiated transfers}, ~\ref{subsubsec:client-initiated transfers} \\ \hline
35 & A server updates the status flag in the UWS Job representation as appropriate & ~\ref{subsubsec:service-initiated transfers}, ~\ref{subsubsec:client-initiated transfers} \\ \hline
36 & A VOSpace service has the REST bindings: properties, views, protocols, nodes, and transfers & ~\ref{sec:rest bindings} \\ \hline
37 & Access policies on a VOSpace service are declared in the registered metadata for the service & ~\ref{sec:access control} \\ \hline
38 & VOSpace authentication employs IVOA SSO supported methods & ~\ref{sec:access control} \\ \hline
...
40 & accepts and provides specify entities that the service supports & ~\ref{subsubsec:getprotocols}, ~\ref{subsubsec:getviews}, ~\ref{subsubsec:getproperties} \\ \hline
41 & An autogenerated URI is specified as a result in the Results List & ~\ref{subsubsec:createnode}*, ~\ref{subsubsec:movenode}*, ~\ref{subsubsec:copynode}*, ~\ref{subsubsec:pushtovospace}* \\ \hline
42 & Any data written to the reserved URI .null is discarded & ~\ref{sec:rest bindings} \\ \hline
43 & accepts is filled in based on service capabilities & ~\ref{subsubsec:createnode} \\ \hline
44 & If a container is deleted then so are its children & ~\ref{subsubsec:deletenode} \\ \hline
45 & If a container is listed, only direct children are listed & ~\ref{subsubsec:getnode} \\ \hline
46 & Results are drawn from the same ordered sequence in any ordering imposed by the server & ~\ref{subsubsec:getnode} \\ \hline
% 47 & Regex expressions comply with POSIX conventions & ~\ref{subsec:searches} \\ \hline
